...
def visualize_recognition_results(image: np.ndarray,
    ...
    for cluster_id, class_name in classifications.items():
        ...
        text_x = max(centroid[0] - text_size[0] // 2, 0)  # make text horizontally
        text_y = centroid[1]  # make vertical position
...
def save_visualization (save_dir: Path, scene_id: str, original_image: np.ndarray, label_image: np.ndarray,
                        result_image: np.ndarray, classifications: Dict[int, str]):
